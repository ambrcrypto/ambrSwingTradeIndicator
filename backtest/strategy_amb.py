...
class AMBParams:
    ...
    # ATR trailing stop (third, independent exit mode)
    trail_sl_enable:    bool  = False
    trail_activate_pct: float = 5.0   # trail activates after X% unrealised gain
    trail_atr_factor:   float = 2.0   # trail distance = ATR(trail_atr_len) × factor
    trail_atr_len:      int   = 14    # ATR period for trail (Wilder's RMA)
    ...
    # Peak-Drawdown stop: exit when close drops X% from the highest close since entry
    # Activates only after the trade has reached peak_dd_activate_pct% unrealised gain
    peak_dd_enable:        bool  = False
    peak_dd_activate_pct:  float = 3.0   # min unrealised gain (%) before PD activates
    peak_dd_pct:           float = 2.0   # exit when close drops X% from peak_close
    ...
    def label(self) -> str:
        ...
        if self.trail_sl_enable:
            sl_str += f"_Trail{self.trail_activate_pct:.0f}x{self.trail_atr_factor:.1f}"
        if self.peak_dd_enable:
            sl_str += f"_PD{self.peak_dd_activate_pct:.0f}dd{self.peak_dd_pct:.1f}"
...
def run_strategy(df: pd.DataFrame, params: AMBParams,
    ...
    trail_sl_level:       float = 0.0   # current trail level; long: ratchets up, short: ratchets down
    trail_reentry_armed:  bool  = False # True after TSL/TSS exit; waits for FastMA bounce

    # Peak-Drawdown stop state
    peak_dd_active:       bool  = False
    peak_close_long:      float = 0.0   # highest close seen during current long
    peak_close_short:     float = float('inf')  # lowest close seen during current short
    ...
    for i in range(1, n):
        ...
        # ── Trail-level update (each bar, before crossover calcs) ──────────────────
        if params.trail_sl_enable and position_open:
            ...
            if last_dir == 1:
                ...
                if trail_active and atr_t > 0:
                    ...
                    trail_sl_level = max(trail_sl_level, new_level)
            elif last_dir == -1:
                ...
                if trail_active and atr_t > 0:
                    new_level = c + atr_t * params.trail_atr_factor
                    trail_sl_level = min(trail_sl_level, new_level)
        # ── Peak-Drawdown update (each bar, before exit calcs) ──────────────────────
        if params.peak_dd_enable and position_open:
            if last_dir == 1:
                unrealised_pct = (c - entry_price) / entry_price * 100.0
                if unrealised_pct >= params.peak_dd_activate_pct:
                    peak_dd_active = True
                if c > peak_close_long:
                    peak_close_long = c
            elif last_dir == -1:
                unrealised_pct = (entry_price - c) / entry_price * 100.0
                if unrealised_pct >= params.peak_dd_activate_pct:
                    peak_dd_active = True
                if c < peak_close_short:
                    peak_close_short = c
        ...
        exit_long_TSL = (params.trail_sl_enable and trail_active
                         and position_open and last_dir == 1  and lo <= trail_sl_level)
        exit_long_PD  = (params.peak_dd_enable and peak_dd_active
                         and position_open and last_dir == 1
                         and peak_close_long > 0
                         and c < peak_close_long * (1.0 - params.peak_dd_pct / 100.0))
        ...
        exit_short_TSL = (params.trail_sl_enable and trail_active
                          and position_open and last_dir == -1 and h  >= trail_sl_level)
        exit_short_PD  = (params.peak_dd_enable and peak_dd_active
                          and position_open and last_dir == -1
                          and peak_close_short < float('inf')
                          and c > peak_close_short * (1.0 + params.peak_dd_pct / 100.0))
        exit_short     = exit_short_A or exit_short_B or exit_short_SL or exit_short_TSL or exit_short_PD

        # ── Entry conditions ────────────────────────────────────────────────
        long_entry   = (not position_open) and (last_dir != 1 or not params.use_fast_ma)  and cross_above_slow
        long_reentry = (not position_open) and (last_dir == 1)  and cross_above_fast and (c > s)

        short_entry   = (not position_open) and (last_dir != -1 or not params.use_fast_ma) and cross_below_slow
        short_reentry = (not position_open) and (last_dir == -1) and cross_below_fast and (c < s)
        # ── Trail re-entry (FastMA bounce after TSL/TSS exit, trend still intact) ────
        trail_reentry_long  = (params.trail_sl_enable and trail_reentry_armed
                               and not position_open and last_dir == 1
                               and cross_above_fast and c > s)
        trail_reentry_short = (params.trail_sl_enable and trail_reentry_armed
                               and not position_open and last_dir == -1
                               and cross_below_fast and c < s)
        # Cancel re-entry if slow MA crossed against us (trend flipped)
        if trail_reentry_armed and last_dir == 1  and cross_below_slow:
            trail_reentry_armed = False
        if trail_reentry_armed and last_dir == -1 and cross_above_slow:
            trail_reentry_armed = False
        ...
        if exit_long and position_open and last_dir == 1:
            if exit_long_TSL:                                               # priority 1: intrabar
                exit_px       = trail_sl_level
                exit_type_str = "TSL"
                pct           = ((trail_sl_level - entry_price) / entry_price * 100.0) * params.leverage_long
                trail_reentry_armed = True
            elif exit_long_SL:                                              # priority 2: intrabar
                exit_px       = c
                exit_type_str = "SL"
            ...
            elif exit_long_PD:                                              # priority 3: close-based
                exit_px       = c
                exit_type_str = "PD"
                pct           = ((c - entry_price) / entry_price * 100.0) * params.leverage_long
            else:                                                           # priority 4: close
                exit_px       = c
                exit_type_str = "CL"
                pct           = ((c - entry_price) / entry_price * 100.0) * params.leverage_long
            ...
            trail_active        = False
            trail_sl_level      = 0.0
            peak_dd_active      = False
            peak_close_long     = 0.0
            peak_close_short    = float('inf')

        elif exit_short and position_open and last_dir == -1:
            if exit_short_TSL:                                              # priority 1: intrabar
                exit_px       = trail_sl_level
                exit_type_str = "TSS"
                pct           = ((entry_price - trail_sl_level) / entry_price * 100.0) * params.leverage_short
                trail_reentry_armed = True
            elif exit_short_SL:                                             # priority 2: intrabar
                exit_px       = c
                exit_type_str = "SL"
                if params.atr_sl_enable:
                    ...
                    pct = -params.sl_risk_pct
            elif exit_short_PD:                                             # priority 3: close-based
                exit_px       = c
                exit_type_str = "PD"
        ...
        if long_signal:
            ...
            pending_short       = False
            trail_active        = False
            trail_sl_level      = 0.0          # long: ratchets upward from 0
            trail_reentry_armed = False
            peak_dd_active      = False
            peak_close_long     = 0.0
            peak_close_short    = float('inf')
            ...
            pending_short       = False
            trail_active        = False
            trail_sl_level      = float('inf')  # short: ratchets downward from inf
            trail_reentry_armed = False
            peak_dd_active      = False
            peak_close_long     = 0.0
            peak_close_short    = float('inf')
